...
if [ "$SKIP_DOWNLOAD" = "NO" ]; then
  ...
  # VERSION_BRANCH=$(curl -s https://xanmod.org/ -A "$CURL_UA" | sed -n "/$BRANCH/{n;n;s/.*<td>\([0-9]\+\.[0-9]\+\)<\/td>.*/\1/p}")  # Anubis
  # 提取 Stable 版本号
  RAW_STABLE=$(curl -s https://www.kernel.org/finger_banner | grep 'latest stable version' | awk '{print $NF}')
  
  # 提取 6.19 这种格式
  # 使用 sed 确保即便没有第三个数字也能正确截断
  VERSION_BRANCH=$(echo "$RAW_STABLE" | sed 's/\([0-9]*\.[0-9]*\).*/\1/')
  
  if [ -z "$VERSION_BRANCH" ]; then
    echo "Error: Could not fetch version from kernel.org"
    exit 1
  fi
